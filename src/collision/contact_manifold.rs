...
use crate::{config::EPSILON, geometry::polygon::compute_outward_edge_normal};
...
fn find_incident_edge_filtered(
    ...
    for edge_index in 0..polygon.len() {
        ...
        let Some(edge_normal) = compute_outward_edge_normal(polygon, edge_index) else {
            continue;
        };
        ...
        // The incident edge is the edge whose outward normal
        // is the most opposite to the reference normal.
        if dot < best_dot {
            best_dot = dot;
...
fn clip_segment_to_line(points: [Vec2; 2], normal: Vec2, offset: f32) -> Vec<Vec2> {
    ...
    let distances = [
        points[0].dot(normal) - offset,
        points[1].dot(normal) - offset,
    ];
...
pub(crate) fn build_contact_manifold(
    ...
    let incident_edge =
        find_incident_edge_filtered(incident_polygon, reference_normal, incident_allowed_edges)?;
