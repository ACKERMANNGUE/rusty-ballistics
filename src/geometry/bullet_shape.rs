...
pub fn get_bullet_world_triangles(
    ...
    let world_triangles = triangles
        ...
        .map(|triangle| {
            ...
            let world_vertices = [
                transform_bullet_vertex(vertices[indices[0]], bullet),
                transform_bullet_vertex(vertices[indices[1]], bullet),
                transform_bullet_vertex(vertices[indices[2]], bullet),
            ];

            WorldTriangle::new(world_vertices, *triangle.get_boundary_edges())
        })
        .collect();
